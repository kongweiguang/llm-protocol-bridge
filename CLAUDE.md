...
- **`UpstreamEndpointResolver`** (`http.client`) — resolves full upstream URL from `ModelConfig` (handles `/v1` suffix normalization).
- **`UpstreamHeaderFactory`** (`http.client`) — constructs HTTP headers per provider (Bearer for OpenAI, x-api-key for Anthropic, plus custom headers).
- **`UpstreamHttpClient`** (`http.client`) — interface for sending requests upstream; `WebClientUpstreamHttpClient` is the reactive Spring WebClient implementation.
...
- All tests are in the `testkit` module, not in individual modules. Tests use AssertJ assertions.
